...
function setupAiAssistant() {
  ...
  trigger.innerHTML = '<span class="ai-assistant__pulse"></span><strong>AI</strong><i></i>'
  ...
  panel.innerHTML = '<header class="ai-assistant__header"><div><strong>Virexo Assistant</strong><span><i></i> Online</span></div><button type="button" class="ai-assistant__close" aria-label="Close Virexo Assistant">×</button></header><div class="ai-assistant__messages" aria-live="polite"></div><div class="ai-assistant__replies"></div>'
  ...
  const menu = [
    ...
    ['How long does a typical project take?', 'timeline'],
    ["What's your pricing range?", 'pricing'],
    ['I want to get a quote', 'quote']
  ]
  const answers = {
  ...
  const addMessage = (text, type = 'assistant') => { const message = document.createElement('p'); message.className = `ai-assistant__message ai-assistant__message--${type}`; message.textContent = text; messages.appendChild(message); messages.scrollTop = messages.scrollHeight }
  ...
  const handle = (label, action) => {
    ...
    if (action === 'menu') { showTyping(() => { addMessage("What would you like to know?"); renderReplies(menu) }); return }
  ...
  const open = () => { lastFocus = document.activeElement; panel.hidden = false; trigger.setAttribute('aria-expanded', 'true'); panel.classList.add('is-open'); messages.innerHTML = ''; replies.innerHTML = ''; showTyping(() => { addMessage("Hi! I'm here to help you explore Virexo Innovations. What would you like to know?"); renderReplies(menu) }); closeButton.focus() }
